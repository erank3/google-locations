  ...
  _self.geocodeAddress(options.geocode, function(err, result){
    ...
    try {
      if(result.results.length == 0) {
          return cb({message: "Error no results", error: {}});
      }
      ...
      return cb({message: "Malformed results -- try a more specific address v1", error: exp});
    }
  });
};
...
function _generateOptions(geocodeOptions) {
  ...
  var keys = ['name', 'maxResults', 'rankby', 'radius', 'types'];
